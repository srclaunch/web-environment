...
async function cleanModels() {
  try {
    ...
    const MODEL_DEPS_PATH = import_path.default.join(import_path.default.resolve(), APPLAB_DIRECTORY, "dependencies/models");
    console.info("Cleaning models...");
    await import_fs_extra.default.emptyDir(import_path.default.join(MODEL_DEPS_PATH, "src"));
  } catch (err) {
    console.error(err);
  }
}
...
async function copyStubModels() {
  try {
    const APPLAB_DIRECTORY = ".applab";
    const BUILD_PATH = import_path2.default.join(import_path2.default.resolve(), APPLAB_DIRECTORY, "dependencies/models");
    const applabModelPath = import_path2.default.join(BUILD_PATH, "src");
    await import_fs_extra2.default.writeFile(import_path2.default.join(applabModelPath, "Organization.ts"), getOrganizationModel(), "utf8");
    await import_fs_extra2.default.writeFile(import_path2.default.join(applabModelPath, "Team.ts"), getTeamModel(), "utf8");
    await import_fs_extra2.default.writeFile(import_path2.default.join(applabModelPath, "Document.ts"), getDocumentModel(), "utf8");
    await import_fs_extra2.default.writeFile(import_path2.default.join(applabModelPath, "Payment.ts"), getPaymentModel(), "utf8");
    await import_fs_extra2.default.writeFile(import_path2.default.join(applabModelPath, "Invoice.ts"), getInvoiceModel(), "utf8");
    await import_fs_extra2.default.writeFile(import_path2.default.join(applabModelPath, "PaymentMethod.ts"), getPaymentMethodModel(), "utf8");
    await import_fs_extra2.default.writeFile(import_path2.default.join(applabModelPath, "Person.ts"), getPersonModel(), "utf8");
    await import_fs_extra2.default.writeFile(import_path2.default.join(applabModelPath, "Subscription.ts"), getSubscriptionModel(), "utf8");
    await import_fs_extra2.default.writeFile(import_path2.default.join(applabModelPath, "User.ts"), getUserModel(), "utf8");
    await import_fs_extra2.default.writeFile(import_path2.default.join(applabModelPath, "UserGroup.ts"), getUserGroupModel(), "utf8");
    await import_fs_extra2.default.writeFile(import_path2.default.join(applabModelPath, "UserRole.ts"), getUserRoleModel(), "utf8");
  } catch (err) {
    console.error(err);
  }
}
...
async function buildAppLabModels({
  ...
  try {
    const MODELS_PATH = import_node_path.default.join(import_node_path.default.resolve(), "models");
    const APPLAB_DIRECTORY = ".applab";
    const BUILD_PATH = import_node_path.default.join(import_node_path.default.resolve(), APPLAB_DIRECTORY, `${projectPath}/src`);
    const files = await import_fs_extra3.default.readdir(MODELS_PATH);
    for (const file of files) {
      const fileContents = await import_fs_extra3.default.readFile(import_node_path.default.join(MODELS_PATH, file), "utf8");
  ...
  `;
      ...
      const updatedFileContents = fileContents.replace("fields: {", `fields: {
        ${entityFields}`);
      await import_fs_extra3.default.writeFile(import_node_path.default.join(BUILD_PATH, file), updatedFileContents, "utf8");
    }
    const buildModels2 = await import_fs_extra3.default.readdir(BUILD_PATH);
    ...
    await import_fs_extra3.default.writeFile(import_node_path.default.join(BUILD_PATH, "index.ts"), indexFileContent, "utf8");
...
async function buildModelTypes({ path: projectPath }) {
  try {
    ...
    for (const file of files) {
      ...
      await import_fs_extra4.default.writeFile(import_path3.default.join(BUILD_PATH, file), fileContents, "utf8");
    }
    ...
    for (const model of [...Object.entries(Models)]) {
      ...
      const filePath = import_path3.default.join(BUILD_PATH, fileName);
      await import_fs_extra4.default.writeFile(filePath, types, "utf8");
      exportStr += getModelExports(model[1]);
    }
    await import_fs_extra4.default.writeFile(import_path3.default.join(BUILD_PATH, "index.ts"), exportStr, "utf8");
...
async function buildSequelizeModels({
  ...
  try {
    ...
    for (const model of [...Object.entries(Models)]) {
      ...
      const fileName = `${modelName}.ts`;
      const filePath = import_path4.default.join(BUILD_PATH, fileName);
      await import_fs_extra5.default.writeFile(filePath, sequelizeModel, "utf8");
    }
    const indexFileContent = constructModelExportIndexScript(Object.keys(Models).map((modelName) => modelName), "sequelize");
    await import_fs_extra5.default.writeFile(import_path4.default.join(BUILD_PATH, "index.ts"), indexFileContent, "utf8");
  ...
  try {
    ...
    for (const file of files) {
      if (file !== "index.ts") {
        ...
        await import_fs_extra6.default.writeFile(import_path5.default.join(BUILD_PATH, `${name}.ts`), reduxSlice, "utf8");
      }
    }
    ...
    await import_fs_extra6.default.writeFile(import_path5.default.join(BUILD_PATH, "index.ts"), indexFileContent, "utf8");
...
async function buildHttpClient({
  ...
  try {
    ...
    await import_fs_extra7.default.writeFile(import_path6.default.join(BUILD_PATH, "index.ts"), indexFileContent, "utf8");
...
async function build({
  ...
  try {
    ...
    if (buildTypes) {
      ...
      const tsConfig = await JSON.parse(import_fs_extra8.default.readFileSync(import_path7.default.join(import_path7.default.resolve(), buildPath, "tsconfig.json"), "utf8"));
      const { options } = import_typescript.default.parseJsonConfigFileContent(tsConfig, import_typescript.default.sys, import_path7.default.join(import_path7.default.resolve(), buildPath));
      const buildFiles = (await import_fs_extra8.default.readdir(import_path7.default.join(import_path7.default.resolve(), buildPath, "src"))).filter((f) => f.endsWith(".ts")).map((file) => {
        return import_path7.default.join(import_path7.default.resolve(), buildPath, "src", file);
      });
      const program = import_typescript.default.createProgram(buildFiles, options);
      const emitResult = program.emit();
      let allDiagnostics = import_typescript.default.getPreEmitDiagnostics(program).concat(emitResult.diagnostics);
      allDiagnostics.forEach((diagnostic) => {
        if (diagnostic.file) {
          let { line, character } = import_typescript.default.getLineAndCharacterOfPosition(diagnostic.file, diagnostic.start);
          let message = import_typescript.default.flattenDiagnosticMessageText(diagnostic.messageText, "\n");
          console.info(`${diagnostic.file.fileName} (${line + 1},${character + 1}): ${message}`);
        } else {
          console.info(import_typescript.default.flattenDiagnosticMessageText(diagnostic.messageText, "\n"));
        }
      });
    }
  } catch (err) {
...
To get help for a specific command type help after the command name, for example:
  $ applab dev help
`;
var cli = (0, import_meow.default)(helpMessage, {
  flags: {},
  importMeta: import_meta
});
(0, import_update_notifier.default)({ pkg: cli.pkg }).notify();
run({
